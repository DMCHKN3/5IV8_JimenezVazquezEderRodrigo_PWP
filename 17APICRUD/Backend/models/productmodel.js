...
class Product {
    ...
    static create(newProduct, result) {
        if (newProduct.categoryid && newProduct.name && newProduct.id) {
            ...
            sql.query('INSERT INTO products (categoryid, name, price, stock) VALUES (?,?,?,?)',
                [newProduct.categoryid, newProduct.name, newProduct.price, newProduct.stock],
                (err, res) => {
                    if (err) {
